...
# Load Data
```{r, message = TRUE}
pitcher_plant_data <- read.csv(here("Data/pitcher_plant_data.csv")) # Label Dataset
```
...
```
---
# Specefic Plant Area Exploration
# Exploration
...
```
---
# Specific Plant Area Exploration
...
p <- ggplot(grouped_data,aes(x = leaf_age_, # Select X variable
  ...
  ggtitle('Organism Abundance over Time (Plant ID A Only)') + # Creates title
...
```{r plot3-out, ref.label="plot3", echo=FALSE, warning=FALSE, message=FALSE}
```
...
pitcher_plant_data %>% # select df
      ggscatterhist( x = "leaf_age_", # Select X variable
                     y = "ants", # Select Y variable
                     color = "Plant.ID.Alpha",  # Colors it by the Plant ID Alpha
                     size = 3, # Sets the size of each point
                     alpha = 0.6, # Sets the opacity of each point
        palette = my_pal, # Select palette
        margin.plot = "boxplot", # selects marginal plot
        xlab = "Number of Ants", # Label X-Axis
        ylab = "Age of Leaf (Days)", #Label Y- Axis
        title = "Ant Abundances Over Time", # Label title
        ggtheme = theme_bw()  # Sets theme
        )
```
]
.pull-right[
...
.pull-right[
```{r plot5-out, ref.label="plot5", echo=FALSE, warning=FALSE, message=FALSE}
```
]
---
